...
class WiFiManager:
        ...
        try:
            with open('wifi.dat', 'r') as f:
                ...
                for line in lines:
                    ...
                    if len(parts) == 2:
                        ssid, password = parts
                        print(f"[wifi_manager]: 讀取 Wi-Fi 設定: SSID={ssid}")
                        return ssid, password
        except Exception as e:
            print(f"[wifi_manager]: 無法讀取 wifi.dat: {e}")
    ...
    def save_wifi_config(self, ssid, password):
        ...
        try:
            ...
            print(f"[wifi_manager]: Wi-Fi 設定已更新: SSID={ssid}, PASSWORD=******")
        except Exception as e:
            print(f"[wifi_manager]: 無法寫入 Wi-Fi 設定: {e}")
    ...
    def connect(self, timeout=60, retry_interval=3):
        ...
        if not self.ssid or not self.password:
            print(f"[wifi_manager]: Wi-Fi 設定檔讀取失敗，啟動 AP 設定模式...")
            self.start_ap_web()
            return None

        if self.wifi.isconnected():
            print("[wifi_manager]: Wi-Fi connected!")
            return self.get_ip_mac()

        print(f"[wifi_manager]: 嘗試連線 Wi-Fi {self.ssid} ...")
        self.disconnect()
        ...
        for retry in range(10):
            if self.wifi.isconnected():
                print("[wifi_manager]: Wi-Fi 連線成功！")
                return self.get_ip_mac()
            print(f"[wifi_manager]: 嘗試連線中... {retry+1}/10")
            time.sleep(2)

        print("[wifi_manager]: Wi-Fi 嘗試連線10次失敗！啟動 AP 設定模式")
        self.start_ap_web()
        return None
    ...
    def get_signal_strength(self):
        ...
        if self.wifi.isconnected():
            ...
            print("[wifi_manager]: Unable to retrieve signal strength.")
            return None
    ...
    def get_ip_mac(self):
        ...
        if self.wifi.isconnected():
            ...
            print(f"[wifi_manager]: Network config: {ip_info}")
            return {"ip": ip_address, "mac": mac_address}
        else:
            print("[wifi_manager]: Wi-Fi 未連線，無法獲取 IP/MAC")
            return None
    ...
    def start_ap_web(self, port=80):
        ...
        print(f"[wifi_manager]: AP 模式啟動: SSID={self.ap_ssid}, 密碼={self.ap_password}")
        print("[wifi_manager]: 開啟瀏覽器，連線到 192.168.4.1 來設定 Wi-Fi")
    ...
    def handle_web_requests(self, client):
        ...
        try:
            ...
            print(f"[wifi_manager]: 處理請求時發生錯誤: {e}")
        finally:
            client.close()  # 確保關閉連線
        ...
        try:
            ...
            for line in response.split("\r\n"):
                if line.startswith("Date: "):
                    ...
                    print(f"[wifi_manager]: Google 時間: {date_str}")
                    ...
                    try:
                        ...
                        print(f"[wifi_manager]: 設備時間已更新: {year}-{month}-{day} {hour}:{minute}:{second}")
                        return True
                    except Exception as e:
                        print(f"[wifi_manager]: 解析時間失敗: {e}")
                        return None
            print("[wifi_manager]: 無法獲取 Google 時間")
            return None
        except Exception as e:
            print(f"[wifi_manager]: Google 時間 API 失敗: {e}")
            return None 